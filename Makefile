...
ADOCFONTSDIR = /var/lib/gems/2.*/gems/asciidoctor-pdf-1.5.*/data/fonts/
TTFFONTSDIR = /usr/share/fonts/truetype/
...
create_epub: create_folder copy_fonts
	export XML_CATALOG_FILES=${XML_CATALOG_FILES}; \
	cd ${DIR}; \
	asciidoctor -a ebookbuild -b docbook -o ${BOOK}.xml ${BOOK}.asciidoc; \
	a2x ${EPUBOPTS} ${BOOK}.xml; \
...
dist_epub: create_epub create_dist
	if [ -f "${DIR}/${BOOK}.epub" ]; then \
		cp ${DIR}/${BOOK}.epub ${DIST}/${LANG}/${TITLE}_${TAG}.epub; \
	fi; \

dist_kindle: create_kindle create_dist
	if [ -f "${DIR}/${BOOK}.mobi" ]; then \
		cp ${DIR}/${BOOK}.mobi ${DIST}/${LANG}/${TITLE}_${TAG}.mobi; \
	fi; \
